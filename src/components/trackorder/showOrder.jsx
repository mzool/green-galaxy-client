  ...
  return (
    <div className="w-page flex-col gap-10 bg-white p-4 min-h-screen rounded-md text-gray-700">
      <div className="p-2 bg-green-600 rounded-md mb-10">
        <button onClick={()=>setOrder({})} className="px-4 py-2 bg-white text-gray-700 rounded-md hover:bg-gray-200 transition duration-300">
          go back
        </button>
      </div>
      {/* order info */}
      ...
      {order.items.map((item, index) => (
        ...
        >
          ...
          <h2>{item.name}</h2>
          {item.color && <div className="h-6 w-6 rounded-md " style={{backgroundColor:item.color}}/>}
